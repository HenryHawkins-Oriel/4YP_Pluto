...
rx_bob(z, frame_size, noise_pwr, freq_corr, real_params)
    %% INITIALISE
    ...
    rxSig_dspr = zeros((real_params.header_len + real_params.payload_len)/real_params.bps, 1);
    ...
    msg_set = zeros(real_params.no_of_msgs*real_params.msg_len, 1); 
    for msg_cnt = 0 : (real_params.no_of_msgs - 1)
        msg_set(msg_cnt * real_params.msg_len + (1 : real_params.msg_len)) = ...
            sprintf('%s %03d\n', real_params.msg, mod(msg_cnt, real_params.no_of_msgs));
    end
    ...
    BER_mask = zeros(real_params.no_of_msgs*length(real_params.msg)*7, 1);
    for i = 1 : real_params.no_of_msgs
        BER_mask((i-1)*length(real_params.msg)*7 + (1:length(real_params.msg)*7)) = ...
        (i-1)*real_params.msg_len*7 + (1:length(real_params.msg)*7);
    end
    ...
    % Set properties
    rx_filter.RolloffFactor = real_params.alpha; % Excess bandwidth
    rx_filter.FilterSpanInSymbols = real_params.filter_span;
    rx_filter.InputSamplesPerSymbol = real_params.interpol;
    rx_filter.DecimationFactor = real_params.decim_fact;
    ...
    % Set properties
    crs_freq_est.Modulation = real_params.mod_type;
    crs_freq_est.Algorithm = 'Correlation-based';
    crs_freq_est.MaximumFrequencyOffset = 6e3;
    crs_freq_est.SampleRate = real_params.f_s/real_params.decim_fact;
    ...
    crs_freq_comp.SampleRate = real_params.f_s/real_params.decim_fact;
    
    %% TIMING RECOVERY
    % Correct symbol timing clock skew
    
    % Phase detector gain (Kp) for Timing Recovery PLL
    % Refer to Eq (7.31) of "Digital Communications - A Discrete-Time Approach" by Michael Rice
    A = 1/sqrt(2);
    K = 1;
    if real_params.M == 2
        K_p = 2.7*2*K*A^2; % 2.7
    ...
    elseif real_params.M == 16
        K_p = 5.4; % Change
    elseif real_params.M == 32
        K_p = 5.4; % Change
    elseif real_params.M == 64
        K_p = 5.4; % Change
    else
    ...
    symbol_sync.SamplesPerSymbol = real_params.interpol/real_params.decim_fact; % Post-filter oversampling
    ...
    % Set properties
    fn_freq_comp.Modulation = real_params.mod_type;
    fn_freq_comp.ModulationPhaseOffset = 'Auto';
    fn_freq_comp.SamplesPerSymbol = real_params.interpol/real_params.decim_fact; % Post-filter oversampling
    ...
    frame_sync.PreambleLength = real_params.header_len * real_params.chip_no(z) / real_params.bps;
    ...
    elseif real_params.M == 4
        ...
        % Set properties
        demodulator.PhaseOffset = pi/real_params.M;
    ...
    descrambler = comm.Descrambler( ...
        real_params.scram_base, ...
        real_params.scram_poly, ...
        real_params.scram_IC);
    ...
    if real_params.M == 2
        ref_const = pskmod(0:real_params.M - 1, real_params.M, 0);
        axis_limits = [-2 2];
    elseif real_params.M == 4 || real_params.M == 8
        ref_const = pskmod(0:real_params.M - 1, real_params.M, pi/real_params.M);
        axis_limits = [-2 2];
    elseif real_params.M == 16
        ref_const = qammod(0:real_params.M - 1, real_params.M);
        axis_limits = [-5 5];
    elseif real_params.M == 32
        ref_const = qammod(0:real_params.M - 1, real_params.M);
        axis_limits = [-7 7];
    ...
    const_diagram.SamplesPerSymbol = real_params.interpol/real_params.decim_fact;
    ...
    rxPluto.RadioID = real_params.Bob_Rx_ID;
    ...
    while current_time <  real_params.stop_time_bob
        ...
        if frame_valid   
            ...
            % Iterate over each symbol
                for j = 1 : (real_params.header_len + real_params.payload_len)/real_params.bps
                    rxSig_dspr(j,1) = (rxSig_frame_sync_T(1,(((j-1)*real_params.chip_no(z))+1:(j*real_params.chip_no(z)))) * chip_seq)/real_params.chip_no(z);
                end
            ...
            ph_ambiguity_est = round(angle(mean(conj(header_mod) .* ...
                rxSig_dspr(1:(real_params.header_len/real_params.bps))))*2/pi)/2*pi;    
            ...
            if real_params.M == 2
                ...
                rxSig_bin = int2bit(rxSig_demod,real_params.bps);
            elseif real_params.M == 4
            ...
            rxSig_descram = descrambler(rxSig_bin(real_params.header_len + (1:real_params.payload_len),1));
    ...
    if txSig_pwr < 0
        ...
        EbNo = SNR*real_params.BW*real_params.T_sym/real_params.bps;
    end
    ...
    if ~isnan(txSig_pwr)
        if real_params.M == 2 || real_params.M == 4 || real_params.M == 8
            % BPSK, QPSK, or 8PSK
            BER_theo = (2/real_params.bps)*qfunc(sqrt(EbNo*2*real_params.bps*(sin(pi/real_params.M))^2));
        elseif real_params.M == 16 || real_params.M == 32 || real_params.M == 64
            % 16-QAM, 32-QAM, or 64-QAM
            BER_theo = (4/real_params.bps)*((sqrt(real_params.M) - 1)/sqrt(real_params.M)) * ...
                qfunc(sqrt(EbNo*3*real_params.bps/(real_params.M - 1))); 
        else
            disp('Please input a valid modulation scheme.')
        end   
    else
